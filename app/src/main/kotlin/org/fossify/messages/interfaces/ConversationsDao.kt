...
interface ConversationsDao {
    ...
    @Query("SELECT (SELECT body FROM messages LEFT OUTER JOIN recycle_bin_messages ON messages.id = recycle_bin_messages.id WHERE recycle_bin_messages.id IS NULL AND messages.thread_id = conversations.thread_id ORDER BY messages.date DESC LIMIT 1) as new_snippet, * FROM conversations WHERE archived = 0 AND is_filtered = 0")
    ...
    fun getAllNonArchivedWithLatestSnippet(): List<ConversationWithSnippetOverride>

    fun getAllNonArchived(): List<Conversation> {
        return getAllNonArchivedWithLatestSnippet().map { it.toConversation() }
    }

    @Query("SELECT (SELECT body FROM messages LEFT OUTER JOIN recycle_bin_messages ON messages.id = recycle_bin_messages.id WHERE recycle_bin_messages.id IS NULL AND messages.thread_id = conversations.thread_id ORDER BY messages.date DESC LIMIT 1) as new_snippet, * FROM conversations WHERE is_filtered = 1")
    fun getFilteredWithLatestSnippet(): List<ConversationWithSnippetOverride>

    fun getFiltered(): List<Conversation> {
        return getFilteredWithLatestSnippet().map { it.toConversation() }
    }

    @Query("SELECT COUNT(*) FROM conversations WHERE is_filtered = 1 AND read = 0")
    fun getUnreadFilteredCount(): Int
    ...
    @Query("UPDATE conversations SET is_filtered = 1 WHERE thread_id = :threadId")
    fun moveToFiltered(threadId: Long)

    @Query("UPDATE conversations SET is_filtered = 0 WHERE thread_id = :threadId")
    fun restoreFromFiltered(threadId: Long)
